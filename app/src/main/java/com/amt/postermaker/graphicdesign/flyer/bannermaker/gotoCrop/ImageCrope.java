...
public class ImageCrope extends FrameLayout {
    ...
    public ImageCrope(Context context) {
        super(context);
        init(context);
    }

    public ImageCrope(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mLayoutWidth > 0 && this.mLayoutHeight > 0) {
            RelativeLayout.LayoutParams origparams = new RelativeLayout.LayoutParams(this.mLayoutWidth, this.mLayoutHeight);
    ...
    public void setImageBitmap(Bitmap bitmap, ExifInterface exif) {
        ...
        switch (exif.getAttributeInt("Orientation", 1)) {
            ...
            case 6:
                ...
                break;
            case 8:
                rotate = VerticalSeekBar.ROTATION_ANGLE_CW_270;
                break;
        }
        if (rotate == -1) {
            setImageBitmap(bitmap);
    ...
    public RectF getActualCropRect() {
        ...
        float scaleFactorWidth = ((float) this.mBitmap.getWidth()) / ((float) displayedImageRect.width());
        float scaleFactorHeight = ((float) this.mBitmap.getHeight()) / ((float) displayedImageRect.height());
        float actualCropLeft = (Edge.LEFT.getCoordinate() - ((float) displayedImageRect.left)) * scaleFactorWidth;
    ...
    private static int getOnMeasureSpec(int measureSpecMode, int measureSpecSize, int desiredSize) {
        ...
        return measureSpecMode == Integer.MIN_VALUE ? Math.min(desiredSize, measureSpecSize) : desiredSize;
    }
}
