...
import com.msl.demo.NetworkQueryHelper.ResponseListener;
...
public class StickerGridActivity extends FragmentActivity {
    ...
    class C05875 implements Comparator<CategoryRowInfo> {
        C05875() {
        }
    ...
    class C09831 implements ViewPager.OnPageChangeListener {
        C09831() {
        }
    ...
    class C09843 implements ResponseListener {
        C09843() {
        }
    ...
    class C09854 implements ResponseListener {
        C09854() {
        }
    ...
    private void initUI() {
        ...
        this.tabHost.setOnPageChangeListener(new C09831());
        findViewById(R.id.btn_back).setOnClickListener(new C05862());
    }

    private void checkDataVersionChanges() {
        this.curVer = this.sharedpreferences.getInt("DataVersion", 0);
        NetworkQueryHelper.getInstance().getServerDbVersion(new C09843());
    }

    private void fetchAndInsertCategory() {
        NetworkQueryHelper.getInstance().getCategoriesData(this.categoriesCount, new C09854());
    }

    private void intiCategoryList(ArrayList<CategoryRowInfo> categoriesList) {
        Collections.sort(categoriesList, new C05875());
        this.adapter = new StickerPagerAdapter(this, getSupportFragmentManager(), categoriesList);
